...
public class NewService {
    ...
    public static JSONObject getbookinfo(String token,String book_id) {
        JSONObject res = null;
        try {
            String path =  rooturl+"index.php?_action=getBookinfo&token="+token+"&book_id="+book_id;
            URL url = new URL(path);
            HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
            // 设置请求的方式
            urlConnection.setRequestMethod("GET");
    ...
    public static int deleteBook(String token,String book_id){
        int res = 0;        //服务器错误
        try {
            String path = rooturl+"index.php?_action=deleteBook&token="+token+"&bookid="+book_id;
            URL url = new URL(path);
            HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
            // 设置请求的方式
            urlConnection.setRequestMethod("GET");
            ...
            if (urlConnection.getResponseCode() == 200) {
                ...
                baos.close();
                System.out.println(baos.toString());
                res = new JSONObject(baos.toString()).getJSONObject("data").getInt("status");
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public static int collection(String token,String bookid){
        int res = 0;        //0代表失败
        try {
            String path = rooturl+"index.php?_action=postCollection&token="+token;
            URL url = new URL(path);
            HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
            // 设置请求的方式
            urlConnection.setRequestMethod("POST");
    ...
    public static JSONObject getMyCollection(String token,int page){
        JSONObject res = null;
        try {
            String path = rooturl+"index.php?_action=getMyCollection&token="+token+"&page="+page;;
            URL url = new URL(path);
            HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
            // 设置请求的方式
            urlConnection.setRequestMethod("GET");
